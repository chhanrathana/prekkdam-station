<div class="card">
    <div class="card-header text-white bg-primary">
        ...
        <strong>{{ __('form.revenue_info') }}</strong>
    </div>
    <div class="card-body">
        <div class="row">
            ...
            <div class="form-group col-sm-4">
                ...
                <select class="form-control select2  {{ $errors->first('revenue_type_id') ? 'is-invalid':'' }}"  name="revenue_type_id" id="revenue_type_id">
                    <option value="" selected>[-- {{ __('form.select') }} --]</option>
                    @foreach ($types as $type)
                        @if ($record)
                            <option value="{{ $type->id }}" {{ $record->revenue_type_id == $type->id ? 'selected' :  '' }} >{{ $type->name_kh }}</option>
                        @else
                            <option value="{{ $type->id }}" {{  old('revenue_type_id') == $type->id ? 'selected' :  '' }} >{{ $type->name_kh }}</option>
                        @endif
                    @endforeach
                </select>
                <div class="invalid-feedback">{{ $errors->first('revenue_type_id') }}</div>
            </div>
    ...
    <div class="card-footer">
        <div class="row">
            <div class="col">
                <a class="btn btn btn-warning float-left" href="{{ route('operation.revenue.index')}}">
