            ...
            <table class="table table-bordered table-striped table-hover" id ="table">
                <thead>
                    <tr>
                        ...
                        <th class="text-center text-nowrap align-middle" colspan="5">{{ __('form.deposit') }}</th>                        
                    ...
                    <tr>                        
                        ...
                        <th class="text-center text-nowrap">{{ __('form.admin_fee_amount') }}</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        <td class="text-center text-nowrap">
                            <div class="btn-group" role="group" aria-label="Action">
                                {{-- <a class="btn btn-sm btn-warning mr-1" target="_blank" href="{{ route('deposit.download',['id' => $record->id ])}}">
                                    <i class="fas fa-print"></i> {{ __('form.btn_print') }}
                                </a> --}}
    
                                <a class="btn btn-sm btn-success mr-1" href="{{ route('operation.deposit.request.show',['id' => $record->id ])}}">
                                    <i class="fas fa-eye"></i> {{ __('form.btn_view') }}
                                </a>
    
                                <a class="btn btn-sm btn-primary mr-1 {{ $record->status <> 'pending'?'disabled':'' }}"
                                    href="{{ route('operation.deposit.request.edit',['id' => $record->id ])}}" type="button">                                    
                        ...
                        <td class="text-right text-nowrap">{{ number_format($record->admin_fee_amount, 2) }} KHR</td>

                        <td class="text-right text-nowrap">{{ number_format($record->balance_amount,2) }} KHR</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
