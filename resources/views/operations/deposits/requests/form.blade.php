<div class="card">
    <div class="card-header text-white bg-primary">
        ...
        <strong>{{ __('form.deposit_info') }}</strong>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="form-group col-sm-4">
                ...
                <select class="form-control select2  {{ $errors->first('deposit_type_id') ? 'is-invalid':'' }}"  name="deposit_type_id" id="deposit_type_id">
                    ...
                    @foreach ($types as $type)
                        @isset ($record)
                            <option value="{{ $type->id }}" {{ $record->deposit_type_id == $type->id ? 'selected' :  '' }} >{{ $type->name_kh }}</option>
                        @else
            ...
            <div class="form-group col-sm-4">
                <label>{{  __('form.interest_rate') }}<span class="text-danger">*</span></label>
                <input
                    class="form-control number {{ $errors->first('interest_rate') ? 'is-invalid':'' }}"
                    name="interest_rate"
                    pattern="[0-9.]+"
                    type="text"
                    placeholder="0.7"
    ...
    <div class="card-footer">
        <div class="row">
            <div class="col">
                <a class="btn btn-warning float-left" href="{{ route('operation.deposit.request.index')}}">
                    <i class="fas fa-arrow-left"></i>  {{ __('form.btn_back') }}
                </a>

                <button class="btn btn-success float-right" type="submit" {{ !$client?'disabled':'' }}>                    
