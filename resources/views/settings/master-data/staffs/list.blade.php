    ...
    <div class="card-header">
        <div class="row">
            <div class="col">
                ...
                <strong>{{ __('form.staff_list') }}</strong>
            </div>
            <div class="col">
                <a class="float-right btn btn-md btn-info" href="{{ route('setting.master-data.staff.create') }}">
            ...
            <table class="table table-bordered table-striped table-hover" id="table">
                <thead>
                    <tr>
                        ...
                        <th class="text-center text-nowrap">{{ __('form.salary_amount') }}</th>
                        <th class="text-center text-nowrap">{{ __('form.start_work_date') }}</th>
                    ...
                    <tr>                         
                        <td class="text-center text-nowrap">
                            <a class="btn btn-sm btn-primary mr-1 "
                                href="{{ route('setting.master-data.staff.edit',['id' => $record->id ])}}" type="button">                                    
                        ...
                        <td class="text-right text-nowrap">{{ number_format($record->salary_amount) }} {{ __('form.khr') }}</td>
                        <td class="text-center text-nowrap">{{ $record->start_work_date??''}}</td>
                        <td class="text-center text-nowrap">{{ $record->_sex->name_kh??''}}</td>
                        <td class="text-center text-nowrap">{{ $record->date_of_birth??''}}</td>
