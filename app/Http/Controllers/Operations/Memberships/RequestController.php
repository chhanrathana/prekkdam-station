...
{    
    public function index(Request $request)
    {
        ...
        return view('operations.memberships.requests.index',[
            'records' => $records,
            'status' => $status,
        ]);
    }
    ...
    {        
        ...
        return view('operations.memberships.requests.create', [
            'client' => $client,
            'clients' => $clients,
    ...
    public function store(DepositRequest $request)
    {
        ...
        try {          
            ...
            $this->depositRequstService->createFristPayment($record);
            DB::commit();
    ...
    public function edit($id)
    {
        ...
        return view('operations.memberships.requests.edit', [
            'record' => $record
        ]);
    }
    ...
    public function update(DepositRequest $request, $id)
    {
        ...
        try {
            $client = Client::find($request->client_id);  
            $record = Deposit::find($request->record_id);
    ...
    {
        $record = Deposit::find($id);           
        return view('operations.memberships.requests.show',[
            'record' => $record,
        ]);
    } 
    ...
    public function destroy($id)
    {
        ...
        try {
            $record = Deposit::find($id);
            DepositPayment::where('record_id', $id)->delete();
            $record->delete();        
            DB::commit();
            return redirect()->back()->with('success', __('message.success'));
        } catch (\Exception $ex) {
