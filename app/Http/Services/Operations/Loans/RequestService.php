...
class RequestService
{
    ...
    {
       $count = Loan::where('branch_id', auth()->user()->branch_id)->count();
        $code = (1 + $count);
    ...
    public function createLoan($request, $client)
    {
        ...
        $loan->fill($request->only([
            'principal_amount', 'term', 'registration_date', 'start_interest_date', 'staff_id','interest_rate','loan_type_id','admin_fee_amount'
        ]));

        $registrationDate = Carbon::createFromFormat('d/m/Y', $request->registration_date)->format('Y-m-d');
                
        $loan->interest_rate = $request->interest_rate;
        $loan->update_balance_date = $registrationDate;
        $loan->balance_amount = $request->principal_amount;
