...
class SathapanaCheckSumService
{
    ...
    public function getSingleRecord($referenceNumber)
    {
        try {
            ...
            $curl = curl_init();
            $url = env('CHECK_SUM_API_SATHAPANA_SINGLE_RECORD');
            ...
            curl_setopt_array($curl, [
                ...
                CURLOPT_HTTPHEADER => [
                    ...
                    "Authorization: " . env('CHECK_SUM_API_SATHAPANA_AUTH'),
                ],
            ]);
    ...
    public function getMultipleRecord($startDatetime, $endDatetime)
    {
        try {
            ...
            $curl = curl_init();
            $url = env('CHECK_SUM_API_SATHAPANA_MULTIPLE_RECORD');
            $url = sprintf("%s?%s", $url, http_build_query($form));
