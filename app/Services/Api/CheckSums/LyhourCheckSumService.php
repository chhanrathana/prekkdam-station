...
class LyhourCheckSumService
{
    ...
    public function getSingleRecord($referenceNumber)
    {
        try {
            ...
            $curl = curl_init();
            $url = env("CHECK_SUM_API_LYHOUR_SINGLE_RECORD");
    ...
    public function getMultipleRecord($startDatetime, $endDatetime)
    {
        try {
            $form = [
                "start_mpwt_transaction_datetime" => Carbon::parse($startDatetime)->format('Y/m/d'),
                "end_mpwt_transaction_datetime" => Carbon::parse($endDatetime)->format('Y/m/d'),
            ];

            $curl = curl_init();
            $url = env("CHECK_SUM_API_LYHOUR_MULTIPLE_RECORD");
            curl_setopt_array($curl, array(
                CURLOPT_URL => $url,
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => "",
            ...
            if ($httpCode == 200) {                
                if (count($data) > 0) {
                    ...
                    $res = [
                        ...
                        'response_end_mpwt_transaction_datetime' => Carbon::parse($data['response_start_mpwt_transaction_datetime'])->format('Y-m-d H:i:s'),
